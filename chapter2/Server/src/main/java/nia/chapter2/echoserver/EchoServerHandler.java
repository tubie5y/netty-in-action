 ...
 * - 因为你的Echo服务器会响应传入的消息, 所以它需要实现 ChannelInboundHandler 接口,用来定义响应入站事件的方法。
 *   这个简单的应用程序只需要用到少量的这些方法, 所以继承ChannelInboundHandlerAdapter类也就足够了,它提供了ChannelInboundHandler的默认实现。
 */
...
public class EchoServerHandler extends ChannelInboundHandlerAdapter {
     ...
     *
     * - ChannelInboundHandlerAdapter 有一个直观的API，并且它的每个方法都可以被重写以挂钩到事件生命周期的恰当点上。
    ...
    /**
     * - channelReadComplete()——通知 ChannelInboundHandler 最后一次对channelRead()的调用是当前批量读取中的最后一条消息；
     */
    @Override
     ...
     *   - 每个Channel都拥有一个与之相关联的 ChannelPipeline，其持有一个ChannelHandler的实例链。
