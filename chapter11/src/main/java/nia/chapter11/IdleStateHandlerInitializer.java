...
/**
 * Listing 11.7 Sending heartbeats (发送心跳)
 ...
 *      - 如果连接超过60秒没有接收或者发送任何的数据，那么IdleStateHandler❶将会使用一个IdleStateEvent事件来调用fireUserEventTriggered()方法。
 *        HeartbeatHandler实现了userEventTriggered()方法，如果这个方法检测到IdleStateEvent事件，它将会发送心跳消息，并且添加一个将在发送操作失败时关闭该连接的ChannelFutureListener❷。
 *
